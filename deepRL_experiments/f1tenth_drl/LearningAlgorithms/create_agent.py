...
# For Quantization
from f1tenth_drl.LearningAlgorithms.td3_quantizable import TestTD3Quant, TrainTD3Quant
...
def create_train_agent(run_dict, state_dim):
    ...
    elif run_dict.algorithm == "TD3Quant":
        agent = TrainTD3Quant(state_dim, action_dim)
...
def create_test_agent(filename, directory, run_dict, state_dim=None):
    action_dim = 2
    
    if run_dict.algorithm == "TD3":
        agent = TestTD3(filename, directory, state_dim, action_dim)
    elif run_dict.algorithm == "TD3Quant":
        agent = TestTD3Quant(filename, directory, state_dim, action_dim)
    elif run_dict.algorithm == "SAC":
