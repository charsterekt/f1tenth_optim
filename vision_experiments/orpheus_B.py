...
import time
from model_preprocessing.utils import preprocess_data, add_to_buffer, is_buffer_full, buffers_to_tensors, prepare_model_for_ptq
from ppc import perception, planning, control
import numpy as np
from torch_classes.orpheus_B import CNNLSTMModel, TimeDistributedCNN
import io
...
model_path = '/Users/chari/autodrive/models/hybrid_ptq.pth'
device = torch.device('cpu')
# device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
print(f"Device: {device}")

model = CNNLSTMModel(seq_length=10)
model = prepare_model_for_ptq(model)
# print(model)
